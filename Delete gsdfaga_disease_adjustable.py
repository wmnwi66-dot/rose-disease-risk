...
def fetch_historical_climate(lat, lon):
    try:
        ...
        # 日別データ：
        # 気温・湿度は長期比較用のERA5-Landを使用する。
        # 重要：ERA5-Landには降水量がないため、降水量だけはERA5から取得する。
        # これが以前のコードで降水量が全て0.0mmになっていた主原因。
        daily_url = (
        ...
        daily_precip_url = (
            ...
            f"&timezone=Asia%2FTokyo&models=era5"
        )

        # 時間別データ：
        # 気温・湿度はERA5-Land、降水量は降水データを持つERA5から取得する。
        hourly_url = (
            f"https://archive-api.open-meteo.com/v1/archive"
            f"?latitude={lat}&longitude={lon}"
            f"&start_date={start_date}&end_date={end_date}"
        ...
        daily_precip_res = requests.get(daily_precip_url, timeout=30)
        daily_precip_res.raise_for_status()
        daily_precip_data = daily_precip_res.json()

        if "daily" not in daily_precip_data:
            return None, "降水量データの取得に失敗しました。"
        ...
        hourly_precip_res = requests.get(hourly_precip_url, timeout=30)
        hourly_precip_res.raise_for_status()
        hourly_precip_data = hourly_precip_res.json()

        if "hourly" not in hourly_precip_data:
            return None, "時間別降水量データの取得に失敗しました。"
        ...
        DATA_INFO.update({
            ...
            "dataset": "ERA5-Land（気温・湿度）＋ ERA5（降水量）",
        })

        daily = daily_data["daily"]
        precip_daily = daily_precip_data["daily"]
        dates = daily["time"]
        precip_dates = precip_daily["time"]
        if dates != precip_dates:
            return None, "日別気象データと降水量データの日付が一致しません。"
        ...
        precip_sums = precip_daily["precipitation_sum"]
        ...
        for i, d_str in enumerate(dates):
            ...
            if precip_sums[i] is not None:
                b["precip_sum_per_year"][y] += max(0.0, float(precip_sums[i]))
        ...
        hp = hourly_precip_data["hourly"]
        hp_times = hp["time"]
        h_precips = hp["precipitation"]

        if h_times != hp_times:
            return None, "時間別気象データと降水量データの時刻が一致しません。"
